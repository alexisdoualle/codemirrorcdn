...
function getTextAndIsolatePositions(str, marks) {
  ...
  for (var i = 0; i < len;) {
    ...
    for (var j = 0; j < marks.length; j++) { if (i >= marks[j].from && i < marks[j].to) { nextIsolate = marks[j]; break; }}
...
function insertLineContent(line, builder, styles) {
  ...
  var nextChange = 0, spanStyle, spanEndStyle, spanStartStyle, title, collapsed, isolate;
  for (;;) {
...
function measureChar(cm, line, ch, bias, atomic) {
...
function measureCharPrepared(cm, prepared, ch, bias, varHeight, atomic) {
...
function charCoords(cm, pos, context, lineObj, bias, atomic) {
...
function drawSelectionRange(cm, range$$1, output) {
  ...
  function drawForLine(line, fromArg, toArg) {
    ...
    iterateBidiSections(order, fromArg || 0, toArg == null ? lineLen : toArg, function (from, to, dir, i, atomic) {
      var ltr = dir == "ltr";
      var fromPos = coords(from, ltr ? "left" : "right", atomic);
      var toPos = coords(to - 1, ltr ? "right" : "left", atomic);
